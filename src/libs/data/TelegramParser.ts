...
export class TelegramParser {
  static parseTelegram(data: Buffer, dpt?: typeof KnxDataDecode.dptEnum[number]) {
    console.log(data)
    if ((data.length - 7) <= KNXTP1.DataLength_Data_In_L_Data_Standard + 1) {
      ...
      const knxDecoder = new KnxDataDecode(apdu)
      const controlField = new ControlField(data[0])
      return {
      ...
      const knxDecoder = new KnxDataDecode(apdu)
      const controlField = new ControlField(data[0])
      const controlFieldExtended = new ExtendedControlField(data[1])
